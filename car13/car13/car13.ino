...
// 平滑左转
void smoothLeftTurn(int speed) {
  analogWrite(LEFT_MOTOR_PIN1, speed*0.3);
  digitalWrite(LEFT_MOTOR_PIN2, LOW);
  analogWrite(RIGHT_MOTOR_PIN1, speed);
  digitalWrite(RIGHT_MOTOR_PIN2, LOW);
}
...
void handleNode() {
  ...
  switch(nodeCount) {
   ...
   case 7:  // 节点7：右转
    ...
    case 10:  // 第十个节点：停止
      Serial.println("执行：检测到节点10：向前前进一段距离");
      moveForward(BASE_SPEED);
      delay(2000);
